    ...
    public partial class Form1 : Form
    {
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand(storedProcedure, conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        List<Song> temp_song_list = songs.Values.ToList();
                        temp_song_list.Insert(
                            0,
                            new Song { SongID = -1, songName = "Select song" }
                        );
                        comboBox17.DataSource = temp_song_list;

                        comboBox27.DataSource = temp_song_list;
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand(storedProcedure, conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        reader.Close();
                        List<Album> temp_album_list = albums.Values.ToList();
                        temp_album_list.Insert(
                            0,
                            new Album { albumID = -1, albumName = "Please Select an Album" }
                        );
                        comboBox20.DataSource = temp_album_list;
                        comboBox20.DisplayMember = "albumName";
                        comboBox20.ValueMember = "albumID";
                        comboBox16.DataSource = temp_album_list;
                        comboBox16.DisplayMember = "albumName";
                        comboBox16.ValueMember = "albumID";
                        comboBox26.DataSource = temp_album_list;
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand(storedProcedure, conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        List<Artist> temp_artist_list = artists.Values.ToList();
                        temp_artist_list.Insert(
                            0,
                            new Artist { artistID = -1, artistName = "Please Select an Artist" }
                        );
                        comboBox13.DataSource = temp_artist_list;
                        comboBox13.DisplayMember = "artistName";
                        comboBox13.ValueMember = "artistID";
                        comboBox6.DataSource = temp_artist_list;
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand(storedProcedure, conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        List<Playlist> temp_playlist_list = playlists.Values.ToList();
                        temp_playlist_list.Insert(
                            0,
                            new Playlist
                            {
                                playlistID = -1,
                                playlistName = "Please Select a Playlist"
                            }
                        );
                        comboBox23.DataSource = temp_playlist_list;
                        comboBox23.DisplayMember = "playlistName";
                        comboBox23.ValueMember = "playlistID";
                        comboBox19.DataSource = temp_playlist_list;
                        comboBox19.DisplayMember = "playlistName";
                        comboBox19.ValueMember = "playlistID";
                        comboBox28.DataSource = temp_playlist_list;
        ...
        private void comboBox16_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (comboBox16.SelectedValue is int selectedAlbumID)
            {
                ...
                comboBox14.DataSource = songsWithoutAlbum_global
                    .Where(song => song.songArtist == artistID.ToString())
                    .ToList();
        ...
        private void button22_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                // Construct the SQL UPDATE command
                string updateCommand =
                    "UPDATE Artist SET ArtistName = @ArtistName WHERE ID = @ArtistID";
        ...
        {
            if (
                comboBox14.SelectedValue is int selectedSongID
                && comboBox16.SelectedValue is int selectedAlbumID
            )
            {
                ...
                if (dataGridView2.Rows.Count > 0)
                {
                    ...
                    {
                        if (
                            row.Cells["songArtist"].Value.ToString()
                            != comboBox15.SelectedValue.ToString()
                        )
                        {
                            x = false;
                        }
        ...
        private void AddSongToPlaylist(int songID, int playlistID)
        {
            try
            {
                ...
                string checkCommand =
                    "SELECT COUNT(*) FROM PlaylistSong WHERE PlaylistID = @PlaylistID AND SongID = @SongID";
        ...
        private void LoadStatistics()
        {
            try
            {
                ...
                using (
                    SqlCommand cmdAvgNumSongsPerArtist = new SqlCommand(
                        queryAvgNumSongsPerArtist,
                        conn
                    )
                )
                using (
                    SqlCommand cmdAvgNumSongsPerAlbum = new SqlCommand(
                        queryAvgNumSongsPerAlbum,
                        conn
                    )
                )
                {
